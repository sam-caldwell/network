...
func (route *RouteInfo) addIPv4Route() (err error) {
	...
	if err := createNetlinkRtAttr(&buffer, unix.RTA_DST, net.IPv4len, route.Gateway.To4()); err != nil {
		return err
	}

	// Add gateway address attribute
	if err := createNetlinkRtAttr(&buffer, unix.RTA_GATEWAY, net.IPv4len, route.Gateway.To4()); err != nil {
		return err
	}
