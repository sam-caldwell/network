...
// Genlmsg represents the Generic Netlink message header (genlmsghdr) as defined in the Linux kernel.
...
func (msg *Genlmsg) Len() (int, error) {
	size := binary.Size(*msg)
	if size < 0 {
		return 0, errors.New("unable to calculate the size of Genlmsg")
	}
	return size, nil
}
