...
// TestIflaIpOverInfinibandEnum tests the size of the IflaIpOverInfinibandEnum type and the values of its enumerated constants.
func TestIflaIpOverInfinibandEnum(t *testing.T) {
	// Verify the size of the IflaIpOverInfinibandEnum type is 1 byte (uint8)
	if size := unsafe.Sizeof(IflaIpOverInfinibandUnspec); size != 1 {
		t.Errorf("Expected size of IflaIpOverInfinibandEnum to be 1 byte, but got %d", size)
	}

	// Test the values of the constants
	tests := []struct {
		name  string
		value IflaIpOverInfinibandEnum
		want  IflaIpOverInfinibandEnum
	}{
