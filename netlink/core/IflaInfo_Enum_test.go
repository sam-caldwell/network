...
// TestIflaInfoEnum tests the size of the IflaInfo type and the values of its enumerated constants.
func TestIflaInfoEnum(t *testing.T) {
	// Verify the size of the IflaInfo type is 1 byte (uint8)
	if size := unsafe.Sizeof(IflaInfoUnspec); size != 1 {
		t.Errorf("Expected size of IflaInfo to be 1 byte, but got %d", size)
	}
